  ...
  p.getCanvasDimensions = function() {
    ...
    dataCurrentByteDim.height = dataCurrentByteDim.maxY-dataCurrentByteDim.minY+1;
		...
		if (dataCurrentByteDim.width > 2) {
			document.getElementById("DataReaderInputCollumn1").className = "collumn2";
			document.getElementById("DataReaderInputCollumn2").className = "collumn2";
			document.getElementById("DataReaderInputCollumn3").className = "collumnNo";
		} else {
			document.getElementById("DataReaderInputCollumn1").className = "collumn3";
			document.getElementById("DataReaderInputCollumn2").className = "collumn3";
			document.getElementById("DataReaderInputCollumn3").className = "collumn3";
		}
...
var funcDataCurrentByte2 = function(p) {
	p.setup = function() {
		if (dataCurrentByteDim===undefined || dataCurrentBytePositions===undefined || dataCurrentBytePositions[0]===undefined || !dataCurrentByte2._setupDone || !QRCode2._setupDone || mask.length < width) {
			setTimeout(dataCurrentByte2.setup, 250);
			return false;
		}
    var c = p.createCanvas(boxWidth*dataCurrentByteDim.width, boxWidth*dataCurrentByteDim.height);
		p.setCurrentMask();
		p.noLoop();
    p.draw();
	};
