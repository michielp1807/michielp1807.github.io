...
var fQRCodePreview = function (p) {
	p.setup = function () {
		p.createCanvas(canvasWidth, canvasWidth);
		p.generateOverlay();
	}

	p.generateOverlay = function () {
		...
		for (i = 0; i < 9; i++) { // generate formatting cells top left
			qrCodeOverlay[i][8] = [140, 191, 255];
		}
		for (j = 0; j < 9; j++) { // generate formatting cells top left
			qrCodeOverlay[8][j] = [140, 191, 255];
		}
		for (j = 0; j < 8; j++) { // generate formatting cells bottom left
			qrCodeOverlay[8][width - j] = [140, 191, 255];
		}
		for (i = 1; i < 9; i++) { // generate formatting cells top right
	...
	p.reload = function () {
		...
		QRCodePreview.generateOverlay();
		QRCodeMask.generateMask();
		dataCurrentByte1.reset();
...
var QRCodePreview = new p5(fQRCodePreview, 'QRCodePreview');
